...
export default function EnquiryList() {
    ...
    const handleDeleteEnquiry = async (userId: string) => {
        if (!confirm('Are you sure you want to delete this user?')) return;

        try {
            const response = await deleteEnquiry(userId);

            if (response.success) {
                setEnquiries(prev => prev.filter(user => user._id !== userId));
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-primary hover:bg-primary">
                                    ...
                                    <TableHead className="text-white" data-testid="header-contact">WhatsApp Update</TableHead>
                                    <TableHead className="text-white" data-testid="header-status">Status</TableHead>
                                    <TableHead className="text-white" data-testid="header-created">Enquired On</TableHead>
                                            ...
                                            <TableCell data-testid={`cell-actions-${index}`}>
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        <DropdownMenuItem
                                                            onClick={() => handleDeleteEnquiry(user._id as any)}
